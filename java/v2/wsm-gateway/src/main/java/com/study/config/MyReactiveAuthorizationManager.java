...
public class MyReactiveAuthorizationManager implements ReactiveAuthorizationManager<AuthorizationContext> {
    ...
    public Mono<AuthorizationDecision> check(Mono<Authentication> authenticationMono, AuthorizationContext authorizationContext) {
        ...
        return authenticationMono.map(auth -> {
            return new AuthorizationDecision(checkAuthorities((Authentication) auth, urls.get(0)));
...
//        return Mono.error(new MyRuntimeException(ResultView.error(ResultEnum.CODE_403)));
    ...
    private boolean checkAuthorities(Authentication auth, String url) {
        Collection<? extends GrantedAuthority> authorities = auth.getAuthorities();//用户拥有的权限
        Collection<ConfigAttribute> configAttributes = map.get(url);//系统中的权限
